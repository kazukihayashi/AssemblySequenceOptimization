...
def Draw(node, connectivity, section, node_color=None, node_vec=None, node_size=None, node_shape=None, front_node_index=None, edge_color=None, edge_annotation=None, scale=4.0, name=0, save=True, show=False):
	"""
	...
	if node.shape[1] == 2 or np.allclose(node[:,2],0): # 2D
		...
		for i in range(np.shape(node)[0]):
			ax.plot([node[i,0]],[node[i,1]], node_shape[i], color=node_color[i], ms=node_size[i]*1.5,clip_on=False)
			if node_shape[i] == "o":
				ax.plot([node[i,0]],[node[i,1]], node_shape[i], color='white', ms=node_size[i],clip_on=False)
		...
		# plot node
		for i in range(np.shape(node)[0]):
			ax.plot(node[i,0],node[i,1],node[i,2], node_shape[i], color=node_color[i], ms=node_size[i]*1.5)
			if node_shape[i] == "o":
				ax.plot([node[i,0]],[node[i,1]],node[i,2], node_shape[i], color='white', ms=node_size[i],clip_on=False)
