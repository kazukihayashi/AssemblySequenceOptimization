    ...
    for gen in range(1500):
        ...
        for i, (child1, child2) in enumerate(zip(parents[::2], parents[1::2])):
            if random.random() < CXPB:
                child1_before = deepcopy(child1)
                child2_before = deepcopy(child2)
                toolbox.mate(child1, child2)
                if not np.all(child1_before==child1):
                    del child1.fitness.values
                    pop_add.append(child1)
                if not np.all(child2_before==child2):
